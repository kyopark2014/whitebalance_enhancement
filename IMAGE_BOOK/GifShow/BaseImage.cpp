...
BOOL CBaseImage::LoadGif(LPCSTR file)
{
	int Width, Height;
	int rWidth;
	int i;	
	...
	m_pGif=new CGif;
	m_pGif->LoadGIF((LPSTR)file);
	
	Width = m_pGif->GetWidth();
	Height = m_pGif->GetHeight();
	rWidth = (Width + 3)/4*4;

	Create(Width,Height,m_pGif->m_Pal);	
	
	for(i=0; i<Height; i++)
		memcpy(&m_pDib[(Height - 1 - i) * rWidth], &m_pGif->m_pData[i * Width], Width);

	delete m_pGif;
	return TRUE;
}

BOOL CBaseImage::SaveGif(LPCSTR file)
{
	int i;
	COLORREF palette[256];
	for(i=0; i<256; i++)
	{
		palette[i]=RGB(lpLogPalette->palPalEntry[i].peRed,
			lpLogPalette->palPalEntry[i].peGreen,
			lpLogPalette->palPalEntry[i].peBlue);
	}		
	m_pGif=new CGif;
	m_pGif->SaveGIF((LPSTR)file,GetSize().cx,GetSize().cy,
				palette,(BYTE *)m_pDib);
	return TRUE;
