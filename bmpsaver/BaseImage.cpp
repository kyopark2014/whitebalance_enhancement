...
BOOL CBaseImage::ImageToDC(SIZE size,LPSTR m_Dib)
{
	int Width,Height;
	int Colornum;
...
//			0,NULL,CREATE_ALWAYS,FILE_ATTRIBUTE_NORMAL,NULL);
	...
	HANDLE fd=CreateFile("capture.bmp",GENERIC_READ|GENERIC_WRITE,
			0,NULL,CREATE_ALWAYS,FILE_ATTRIBUTE_NORMAL,NULL);
	if(!WriteFile(fd,(LPSTR)&bmpHeader,sizeof(bmpHeader),&len,NULL))
		return FALSE;
	msize=((Colornum-1) * sizeof(RGBQUAD))+sizeof(BITMAPINFO);
	if(!WriteFile(fd,pBitmapInfo,msize,&len,NULL))
		return FALSE;
	if(!WriteFile(fd,m_Dib,pBitmapInfo->bmiHeader.biSizeImage,&len,NULL))
		return FALSE;
